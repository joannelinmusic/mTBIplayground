...
# num_of_img_per_patient = df.loc[df['Seperation mark'] == ("Yes")].groupby('patientID').size().reset_index(name='Number of Images')

# num_of_img_per_patient_all = num_of_img_per_patient.loc[df[column_name].notnull()]
# print("Max:", num_of_img_per_patient_all.max(), "Min:", num_of_img_per_patient_all.min())

#Count number of images per patient in T1 (yes)
# num_of_img_per_patient_yes = df.loc[df['Seperation mark'] == ("No")].groupby('patientID').size().reset_index(name='Number of Images')
